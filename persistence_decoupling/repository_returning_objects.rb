=begin
  ...
  * domain objects don't need to know how to (de)serialize themselves

  Cons:
  * there is bidirectional dependency between persistence and domain (see ConferenceRoom#find_by_room_number and ConferenceRoomRepository#find_by_room_number)
...
class ConferenceRoom
  def self.find_by_room_number(number)
    ConferenceRoomRepository.find_by_room_number(number)
  end
...
ConferenceRoomRepository.clas_eval do
  ...
  def self.find_by_room_number(number)
    data_hash = implement_concrete_storage_retrieval(number)
    new_from_hash ConferenceRoom, data_hash
  end

  def self.update(room)
    implement_concrete_storage_save to_hash(room)
  end
end
